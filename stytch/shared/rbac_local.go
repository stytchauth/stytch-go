...
func PerformAuthorizationCheck(
	...
	subjectRoles []string,
	subjectOrgID string,
	authorizationRequest *sessions.AuthorizationCheck,
) error {
	if subjectOrgID != authorizationRequest.OrganizationID {
		...
		if doesListOfStringsContainTarget(subjectRoles, role.RoleID) {
			for _, permission := range role.Permissions {
				hasMatchingAction := doesListOfStringsContainTarget(permission.Actions, "*") ||
					doesListOfStringsContainTarget(permission.Actions, authorizationRequest.Action)
				hasMatchingResource := permission.ResourceID == authorizationRequest.ResourceId
