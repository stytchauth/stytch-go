...
import (
	...
	"github.com/stytchauth/stytch-go/v9/stytch/consumer/m2m/clients/secrets"
...
// update all usages of `client_secret` to `next_client_secret` and call the secret rotation endpoint.
//
...
// RotateCancel: Cancel the rotation of an M2M client secret. After this endpoint is called, the client's
// `next_client_secret` is discarded and only the original `client_secret` will be valid.
...
// Rotate: Complete the rotation of an M2M client secret. After this endpoint is called, the client's
// `next_client_secret` becomes its `client_secret` and the previous `client_secret` will no longer be
// valid.
